...
def quote_ident(ident: str) -> str:
    ...
    # This function is correct for *source* tables. For *output* tables, we must ensure the identifier
    # is manually converted to uppercase before calling this function to get an unquoted identifier.
...
def fetch_current_context(conn) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    try:
        # Use uppercase column names to match Snowflake's default behavior for unquoted identifiers
        df = run_query_df(conn, "select current_warehouse(), current_database(), current_schema()")
        # Access by index since column names might vary slightly (e.g., CURRENT_WAREHOUSE())
        row = df.iloc[0]
...
def connection_sidebar() -> Optional[Any]:
    ...
    with st.sidebar:
        ...
        if "current_warehouse" not in st.session_state or "current_database" not in st.session_state or "current_schema" not in st.session_state:
            # fetch_current_context is now defined before this function's usage, fixing the NameError.
...
def harmonization_workflow(conn, table_a_fqn: str, table_b_fqn: str):
    ...
    with ui_card("3) Cortex AI Recommendations"):
        ...
        with ui_card("5) Create Datasets for Matching Algorithm"):
            # Determine available fields present in both mappings
            candidate_fields = [f for f in ["product_id", "product_name", "product_description", "manufacturer"] if (map_a.get(f) and map_b.get(f))]
            prev_selected = st.session_state.get("final_mapping", {}).get("include_fields", [])
            default_selection = prev_selected if prev_selected else candidate_fields
            include_fields = st.multiselect(
                "Select fields to concatenate and include in model inputs (from mapped fields present in both tables)",
                options=["product_id", "product_name", "product_description", "manufacturer"],
                default=default_selection,
                help="Only fields mapped in both datasets are recommended.",
            )
            if not include_fields:
                st.info("Select at least one field to include.")
            else:
                # Show previews BEFORE running creation by projecting from source tables
                # Clean text by removing special characters, alias columns in UPPERCASE
                def build_preview_select_list(db: str, schema: str, table: str, mapping: Dict[str, Optional[str]], original_label: str) -> str:
                    qtbl = f"{quote_ident(db)}.{quote_ident(schema)}.{quote_ident(table)}"
                    exprs: List[str] = [f"{repr(f'{db}.{schema}.{table}')} as {quote_ident(original_label.upper())}"]
                    concat_parts: List[str] = []
                    for fld in ["product_id", "product_name", "product_description", "manufacturer"]:
                        ...
                        exprs.append(f"{cleaned} as {quote_ident(src.upper())}")
                        concat_parts.append(f"coalesce({cleaned}, '')")
                    concat_expr = "''" if not concat_parts else " || ".join(concat_parts)
                    exprs.append(f"{concat_expr} as {quote_ident('COLLATED_COLUMN')}")
                    return ",\n        ".join(exprs)

                # Build both select lists from current mappings
                sel_ref_prev = build_preview_select_list(db_a, schema_a, tbl_a, map_a, "ORIGINAL_REFERENCE_TABLE")
                sel_inp_prev = build_preview_select_list(db_b, schema_b, tbl_b, map_b, "ORIGINAL_INPUT_TABLE")

                preview_choice_pre = st.selectbox(
                    "Select a Table to Preview",
                    ["None", "Reference Model", "Input Model"],
                    index=0,
                    key="mi_preview_choice_before",
                )
                try:
                    if preview_choice_pre == "Reference Model":
                        df_prev = run_query_df(conn, f"select\n        {sel_ref_prev}\nfrom {quote_ident(db_a)}.{quote_ident(schema_a)}.{quote_ident(tbl_a)} limit 10")
                        st.dataframe(df_prev, use_container_width=True, hide_index=True)
                    elif preview_choice_pre == "Input Model":
                        df_prev = run_query_df(conn, f"select\n        {sel_inp_prev}\nfrom {quote_ident(db_b)}.{quote_ident(schema_b)}.{quote_ident(tbl_b)} limit 10")
                        st.dataframe(df_prev, use_container_width=True, hide_index=True)
                except Exception as e:
                    st.warning(f"Failed to render preview: {e}")
            ...
            # Ensure predicted names are uppercase for the tooltip
            _tbl_a_upper = tbl_a.upper()
            _tbl_b_upper = tbl_b.upper()
            _pred_ref = f"{_tbl_a_upper}_HARMONIZATION_{_today_tag}"
            _pred_inp = f"{_tbl_b_upper}_HARMONIZATION_{_today_tag}"
            
            # Ensure target FQN components are uppercase for the tooltip
            _target_db_upper = st.session_state.get('mi_target_db', '') or (st.session_state.get('current_database') or db_a)
            _target_schema_upper = (st.session_state.get('mi_target_schema', '') or (st.session_state.get('current_schema') or schema_a)).upper()
            ...
            run_build = st.button(
                ...
                help=(
                    f"Creates {_target_db_upper}.{_target_schema_upper}.{_pred_ref} and "
                    f"{_target_db_upper}.{_target_schema_upper}.{_pred_inp}. "
                    f"Also writes audit table {_target_db_upper}.{_target_schema_upper}.AUDIT_HARMONIZATION_{_today_tag}."
                ),
            )
            if run_build:
                with st.spinner("Creating Input & Reference Models and writing audit table..."):
                    try:
                        ref_out, inp_out, audit_out = create_model_inputs_and_audit(
                            conn,
                            target_db,
                            target_schema, # Pass user's schema input
                            table_a_fqn,
                            table_b_fqn,
                            map_a,
                            map_b,
            ...
            if ref_created and inp_created:
                ...
                try:
                    # Note: We expect chosen_fqn to already be in the desired unquoted format: DB.SCHEMA.TABLE
                    db_p, sch_p, tbl_p = parse_fq_table_name(chosen_fqn)
                    chosen_quoted = f"{quote_ident(db_p)}.{quote_ident(sch_p)}.{quote_ident(tbl_p)}"
                except Exception:
                    # Fallback if parse fails, though it shouldn't if creation worked
            ...
            with ui_card("7) Create Harmonized Table"):
                ...
                if st.button(f"Create/Replace {TARGET_TABLE_NAME}", type="primary"):
                    try:
                        ...
                        # NOTE: TARGET_TABLE_NAME is already uppercase, so this quote_ident() call is fine.
...
def create_harmonized_table(
    ...
    # *** FIX: Ensure target schema is uppercase to result in an unquoted identifier ***
    ...
    def select_list(db: str, schema: str, table: str, mapping: Dict[str, Optional[str]], source_label: str, is_table_a: bool) -> Tuple[str, List[str]]:
        ...
        for idx, (alias, col_key) in enumerate(unified_cols):
            ...
            if is_table_a and fuzzy_fields and col_key in fuzzy_fields and mapping.get(col_key):
                ...
                # TMP_FUZZY_MAP_{field_name.upper()} is already uppercase
...
def create_model_inputs_and_audit(
    ...
    # *** FIX 1: Ensure target schema is uppercase to result in an unquoted identifier ***
    target_schema = target_schema.upper()
    ...
    # *** FIX 2: Ensure the generated table name is uppercase to result in an unquoted identifier ***
    ref_out_table = f"{tbl_a}_HARMONIZATION_{date_tag}".upper()
    inp_out_table = f"{tbl_b}_HARMONIZATION_{date_tag}".upper()
    
    # Construct FQNs using the now-uppercase target_schema and output table names
    ...
    # Helper to build SELECT list and concatenation for a side
    def build_side_select(db: str, schema: str, table: str, mapping: Dict[str, Optional[str]], original_label: str) -> Tuple[str, List[str]]:
        qtbl = f"{quote_ident(db)}.{quote_ident(schema)}.{quote_ident(table)}"
        ...
        for field in ["product_id", "product_name", "product_description", "manufacturer"]:
            ...
            concat_parts.append(f"coalesce({cleaned}, '')")
            included_aliases.append(src.upper())
        concat_expr = "''" if not concat_parts else " || ".join(concat_parts)
        select_exprs.append(f"{concat_expr} as {quote_ident('COLLATED_COLUMN')}")
        return ",\n        ".join(select_exprs), included_aliases

    ref_select_list, ref_cols = build_side_select(db_a, schema_a, tbl_a, map_a, "ORIGINAL_REFERENCE_TABLE")
    inp_select_list, inp_cols = build_side_select(db_b, schema_b, tbl_b, map_b, "ORIGINAL_INPUT_TABLE")
...
"""
    ...
    for field in ["product_id", "product_name", "product_description", "manufacturer"]:
        ...
        matched = "Yes" if (field in include_fields and ref_col and inp_col) else "No"
        values_rows.append(
            "("
            + ", ".join([
                ...
                # These string literals will now reflect the uppercase table names
    ...
    # Return the unquoted, uppercase FQNs for the success message
